...
\end{lstlisting}
기본적으로 Darknet은 이미지 로딩을 위해 stb-image.h 를 사용합니다. 또한 OpenCV를 사용하면 디스크에 저장하지 않아도 이미지와 탐지 기능을 볼 수 있습니다.
...
(경로설정을 C:\textbackslash opencv3.0 \textbackslash opencv \textbackslash build\textbackslash include 

C:\textbackslash opencv3.0\textbackslash opencv\textbackslash build\textbackslash x64\textbackslash vc14\textbackslash lib ) 으로 해줍니다 그후에
MSVS2015를 실행하고 build\textbackslash darknet\textbackslash darknet.sln 의 설정을 x64, Release 설정은 Build \textrightarrow Build 로 바꾸어 줍니다.

2. C :\textbackslash \verb opencv_3.0\textbackslash opencv\textbackslash build\textbackslash x64\textbackslash vc14\textbackslash  bin에있는 opencv-world320.dll
및 opencv-ffmpeg320-64.dll 파일을 찾아 darknet.exe 파일과 함께 둡니다.
...
4. MSVS 2015 및 OpenCV 3.0 (경로 : C :\textbackslash \verb opencv_3.0\textbackslash opencv\textbackslash 
build\textbackslash include 
및 C :\textbackslash \verb opencv_3.0\textbackslash opencv\textbackslash build\textbackslash x64\textbackslash 
vc14\textbackslash lib)
에있는 경우 MSVS를 시작하고 

build\textbackslash darknet\textbackslash \verb darknet_no_gpu.sln을 열고 x64 및 Release를 설정하고 다음을 수행하시면됩니다.
그리고 빌드설정은 빌드 \textrightarrow 빌드 darknet 로 하면 됩니다.
...
5. 3.0 대신 OpenCV 2.4.13을 사용하고 있다면 \textbackslash darknet.sln을 연 후 pathes를 변경해야합니다

- (우측클릭 속성) \textrightarrow 속성(properties) \textrightarrow C/C++ \textrightarrow General \textrightarrow Additional Include Directories:
C:\textbackslash \verb opencv_2.4.13\textbackslash opencv\textbackslash build\textbackslash include

- (우측클릭 속성) \textrightarrow 속성(properties) \textrightarrow Linker \textrightarrow General \textrightarrow Additional Library Directories:
C:\textbackslash \verb opencv_2.4.13\textbackslash opencv\textbackslash build\textbackslash x64\textbackslash vc14\textbackslash lib
...
-open \textbackslash darknet.sln \textrightarrow (right click on project) \textrightarrow properties 
...
다운받은 소스에 보면 여러 가지 설정의 Visual Studio (VS) 프로젝트 파일들이 제공됩니다. 
...
한 가지 해결법은 다음과 같습니다. (1) 먼저, *.sIn 파일을 삭제한다. 
(2) 이후 *.vcxproj 파일을 메모장으로 열어서 프로젝트의 버전을 자신의 VS에 맞게 수정해 준다. 
(3) 이후 수정된 *.vcxproj를 클릭하여 프로젝트를 연다.
(프로젝트를 저장하면 *.sIn 파일이 자신의 버전에 맞게 자동 생성됩니다)
...
\begin{figure}[h!]
\centering
\includegraphics[scale=1.0]{enhanceyolo.jpg}
\caption{R-CNN 구조}
\label{fig:detect}
\end{figure}
